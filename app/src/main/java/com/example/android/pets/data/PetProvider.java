...
public class PetProvider extends ContentProvider {
    ...
    private Uri insertPet(Uri uri, ContentValues values) {
        ...
        if (name == null || name.equals("")) {
            Toast toast = Toast.makeText(getContext(), "Give name" + name, Toast.LENGTH_LONG);
            toast.show();
            throw new IllegalArgumentException("Pet requires a name");
        }

        // Check that the breed is not null, put UNKNOWN if null
        String breed = values.getAsString(PetEntry.COLUMN_BREED);
        if (breed.equals("Unknown")) {
            Toast toast = Toast.makeText(getContext(), "Breed given: " + breed, Toast.LENGTH_LONG);
            toast.show();
//            values.put(PetEntry.COLUMN_BREED, "Unknown");
        }

        // Check that the weight is not 0
        int weight = values.getAsInteger(PetEntry.COLUMN_WEIGHT);
        if (weight < 0) {
//            throw new IllegalArgumentException("Pet requires a weight value");
            throw new IllegalArgumentException("Pet requires valid weight");
        }

        // Check if gender is unidentified and display a message if so
